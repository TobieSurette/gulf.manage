...
# Star Oddi data:
vars <- c("temperature", "depth.star.oddi")
ix <- rep(FALSE, nrow(r))
for (i in 1:length(vars)) ix <- ix | !is.na(r[,vars[i]])
star.oddi <- r[which(ix), c("date", "time", vars)]
names(star.oddi) <- gsub("depth.star.oddi", "depth", names(star.oddi))
...
# Write Star Oddi:
write.csv(star.oddi, row.names = FALSE, 
          file = "W:/Lobster/Northumberland Strait/NS_Survey/2024/star oddi/star.oddi.2024.csv")
