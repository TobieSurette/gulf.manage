...
x$species.logbook <- gsub("he[r]*ing", "herring", x$species.logbook) 
...
x$species.logbook[intersect(rep("[eé]ponge", x$species.logbook), rep(c("puant", "grosse", "branche", "pue", "filament", "fine", "carr", "brain", "patte", "non", "speci", "tentacule"), x$species.logbook, and = FALSE))] <- "eponge"
...
x$species.logbook[x$species.logbook == "red fish"]              <- "redfish"
x$species.logbook[x$species.logbook == "sebaste"]               <- "redfish"
...
x$species.logbook[x$species.logbook == "barbu"]                 <- "white hake"
...
x$species.logbook[x$species.logbook == "alligator fish"]     <- "alligatorfish"
...
x$species.logbook[x$species.logbook == "ecaille de palourde"] <- "empty shells"
...
for (i in 1:nrow(ux)){
   ...
   if (length(ix) > 1){
      print(x[ix, ])
      if (all(is.na(as.numeric(x$number.caught[ix])))) s <- NA else s <- sum(as.numeric(x$number.caught[ix]), na.rm = TRUE)
