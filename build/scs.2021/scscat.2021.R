...
# Files to treat:
files <- c("fish", "inverts", "others", "unknown")
files <- paste0("scs.", files, ".", year, ".csv")
path <- "build/scs.2021/data/"
x <- NULL
for (i in 1:length(files)){
   y <- read.csv(paste0(path, files[i]), header = TRUE, stringsAsFactors = FALSE)
   names(y) <- tolower(names(y))
   y$tow.id <- toupper(y$gpnum)
   if (i == 1) x <- y else x <- cbind(x, y[setdiff(names(y), names(x))])
}
